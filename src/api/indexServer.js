...
class IndexService {
  ...
  updateProduct (name, code, normName, sku, sn, Id) {
    // 修改商品信息
    return server({
      url: '/api/v1/product/updateProduct',
      method: 'post',
      data: {
      name,code,normName,sku,sn,Id}
    })
  }
  ...
  productDetails (sn, sku, startDate, endDate, pageStart, pageSize, warehouseId, productId) {
    // 获取商品明细
    return server({
      url: '/api/v1/product/productDetails',
      method: 'post',
      data: {
      sn,sku,startDate,endDate,pageStart,pageSize,warehouseId,productId}
    })
  }
